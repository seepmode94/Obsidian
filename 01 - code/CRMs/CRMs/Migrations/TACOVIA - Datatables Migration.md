...
```sh
GRANT ALL PRIVILEGES ON tacovia_suitecrm_db.* TO 'adminDB_suite'@'localhost';
```
...
1. Populate tacovia_suitecrm_db with the SQL file of current production (Sugar CRM)

```sh
mysql -u adminDB_suite -p -e "DROP DATABASE tacovia_suitecrm_db; CREATE DATABASE tacovia_suitecrm_db;"
mysql -u adminDB_suite -p tacovia_suitecrm_db < tacovia_sugarcrm_db_20241102.sql
```


2. Truncate Tables from database _tacovia_suitecrm_db_ :
...
```sh
mysql -u adminDB_suite -p -e "USE tacovia_suitecrm_db; TRUNCATE TABLE tracker; TRUNCATE TABLE sugarfeed;"
```


3. Drop the old tables from the Production database _tacovia_suitecrm_db_ :

```sh
mysql -u adminDB_suite -p -e "USE tacovia_suitecrm_db; DROP TABLE IF EXISTS acl_actions, acl_roles, acl_roles_actions, aor_reports, aos_pdf_templates, aos_product_categories, aos_products, aos_products_cstm, aow_actions, aow_conditions, aow_workflow, config, email_templates, fields_meta_data, oauth2clients, oauth2tokens, os_reports, outbound_email, relationships, schedulers, securitygroups, securitygroups_acl_roles;"
```

4. Create a temporary database to import the newer SQL file (Seepmode DB):  

```sh
mysql -u adminDB_suite -p -e "DROP DATABASE temp_seepmode_db; CREATE DATABASE temp_seepmode_db;"
...
```sh
mysqldump -u adminDB_suite -p temp_seepmode_db acl_actions acl_roles acl_roles_actions aor_reports aos_pdf_templates aos_product_categories aos_products aos_products_cstm aow_actions aow_conditions aow_workflow config email_templates fields_meta_data oauth2clients oauth2tokens os_reports outbound_email relationships schedulers securitygroups securitygroups_acl_roles > specific_tables.sql
```

6. Import these tables into the original database (seepmode_suitecrm_db):  

```sh
mysql -u adminDB_suite -p tacovia_suitecrm_db < specific_tables.sql
```

7. Export the entire updated original database to a SQL file (if needed): 

```sh
mysqldump -u adminDB_suite -p tacovia_suitecrm_db > tacovia_suitecrm_db_updated.sql  
```
